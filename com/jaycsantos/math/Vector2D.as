package com.jaycsantos.math 
{
 	...
 	public class Vector2D
	{
		...
		{
 			var toTarget:Vector2D = pos2nd.clone(); toTarget.subtract(pos1st);
 			toTarget.normalize();
 			return facing1st.dotOf(toTarget) >= FastMath.cos(fov/2.0);
 		}
		...
		{
 			var xPart:Number = FastMath.cos( a_rotInRadians );
 			var yPart:Number = FastMath.sin( a_rotInRadians );
		...
		/**
		 * An easy way to clone the x/y values from another vector
		 * @param	pos
		 */
		public function moveTo( pos:Vector2D ):Vector2D
		{
			x = pos.x;
			y = pos.y;
			return this;
 		...
 		public function Set(x:Number, y:Number):Vector2D
		{
 		...
 		/**
 		 * Easy way to make a clone of this Vector 
		...
		{
 			v1 = clone(); v1.normalize();
 			v2 = vector.clone(); v2.normalize();
 		...
 		public function dotOf( vector:Vector2D ):Number
		{
 		 ...
 		 * lefthand normal of the other. Also known as perp product.
		...
		{
			//var t:Number = this.dotOf( vector ) / vector.dotOf(vector);
 			//return new Vector2D( x * t, y * t );
			var v:Vector2D = vector.getNormalized();
			v.multiply( dotOf(v) )
			return v;
 		}
 		...
 		public function distanceTo(vector:Vector2D):Number
		{
			...
			var ySep:Number = vector.y - y;
 			return FastMath.sqrt( ySep * ySep + xSep * xSep );
 		}
		...
		/**
		 * Use to find the point where this vector will intersect on another vector
		 * @param	vector
		 * @return
		 */
		public function intersectTo( vector:Vector2D ):Vector2D
		{
			if ( isParallelTo(vector) )
				return new Vector2D( Number.POSITIVE_INFINITY, Number.POSITIVE_INFINITY );
			
			var v1:Vector2D = new Vector2D( vector.x - x, vector.y - y );
			var t:Number = v1.crossOf(vector) / crossOf(vector);
			return new Vector2D( x * t, y * t );
		}
		
		/**
		 * Use to find the point on this vector where another vector will intersect
		 * @param	vector
		 * @return
		 */
		public function intersectFrom( vector:Vector2D ):Vector2D
		{
			if ( isParallelTo(vector) )
				return new Vector2D( Number.POSITIVE_INFINITY, Number.POSITIVE_INFINITY );
			
			var v1:Vector2D = new Vector2D( x - vector.x, y - vector.y );
			var t:Number = v1.crossOf(this) / vector.crossOf(this);
